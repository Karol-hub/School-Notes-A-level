...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=2cm, align=center]
    \node (start) [startstop] {Start};
    \node (forloop) [process, below of=start] {for (int)amountOfRooms * deletingRoomsFactor};
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[node distance=2cm, align=center]
    ...
    \node (checkcondition) [decision, below of=outerloop, yshift=-0.5cm] {if it doesn't exist};
    ...
    \draw [arrow] (checkcondition.west) -- ++(-1,0) node[anchor=west, yshift=-0.2cm] {false} -- ++(0,2.5) -- (outerloop.west);
    \draw [arrow] (newsection.east) -- ++(1,0) -- node[anchor=west, yshift=-0.2cm] {loop \\ back} ++(0,5.5) -- (outerloop.east);
...
\begin{figure}
  ...
  \begin{tikzpicture}[node distance=2cm,align=center]
    ...
    \node (checkexclude) [decision, below of=outerloop, yshift=-0.5cm] {if exclude does \\ not contain point};
    ...
    \node (addnewpoint) [process, below of=innerloop] {if point doesn't \\ exist in NewPoints};
    \node (addtoexclude) [process, right of=addnewpoint, xshift=3cm] {exclude.Add(newPoint)};
    \node (returnexclude) [startstop, below of=addtoexclude] {return exclude};
    ...
    \draw [arrow] (checkexclude.south) -- ++(0,-0.5) node[anchor=east, yshift=-0.2cm] {false} -| ([xshift=-0.5cm,yshift=0.5cm]returnexclude.north);
    \draw [arrow] (branchout) -- (innerloop);
    \draw [arrow] (innerloop) -- (addnewpoint);
    \draw [arrow] (addnewpoint) -- (addtoexclude);
    \draw [arrow] (addtoexclude.south) -- ++(0,-0.5) node[anchor=east, yshift=-0.2cm] {Loop back} -| ([xshift=-0.5cm,yshift=0.5cm]outerloop.north);
    \draw [arrow] (addnewpoint.east) -- ++(1,0) |- (innerloop.east);
    \draw [arrow] (addtoexclude) -- (returnexclude);
  \end{tikzpicture}
  \caption{Flowchart for the given pseudocode}
  \label{fig:makesection-flowchart}
\end{figure}
...
\end{figure}
The pink lines are the edges before the rooms were generated. The green boxes are the result of this algorithm plus the generate corridors algorithm but it shows the algorithm well so it's included in this section.
